...
public:
    ModelARX(const vector<double> &a, //wektor współczynników A
        ...
        , opoznienie(opoznienie) //inicializacja pól
    {
    ...
    {
        for (int i = IleB + opoznienie - 1; i > 0;
             --i) { //przesunięcie tablicy z historią wartości sterujących o jeden
        ...
        for (int i = 0; i < IleB; ++i) {
            ...
            if (OpozSter < 20) //zabezpieczenie przed wyjściem poza zakres
                WartWyjsc
                    += B[i]
                       * HistSter[OpozSter]; //sumowanie wgodnie ze wzorem  (b0ui-k + b1ui-k-1 + … + bdBui-k-dB)
        }

        for (int i = 0; i < IleA; ++i) {
            WartWyjsc
                -= A[i]
                   * HistWyjsc[i]; //odejmowanie od wartości wyjścia zgodznie ze wzorem sumy (a1yi-1 + a2yi-2 + … + adAyi-dA)
        }

        WartWyjsc
            += losowySzum(); // szum dodany do wyjścia składając wszystko otrzymujemy cały wzór yi = (b0ui-k + b1ui-k-1 + … + bdBui-k-dB) - (a1yi-1 + a2yi-2 + … + adAyi-dA) + zi,
